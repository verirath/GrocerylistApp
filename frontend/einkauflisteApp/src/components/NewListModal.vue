...
import { ref } from "vue";
import type { CreateListPayload, NewItemPayload } from "../api/shoppingListApi";

const emit = defineEmits<{
  (e: "close"): void;
  (e: "submit", payload: CreateListPayload): void;
}>();
...
async function handleSubmit() {
  if (!name.value.trim()) {
    errorMessage.value = "Bitte einen Namen für die Einkaufsliste angeben.";
    return;
  }

  const preparedItems = items.value
  ...
  const payload: CreateListPayload = {
    ...
    items: preparedItems.length > 0 ? preparedItems : undefined,
  };
...
<template>
  <div class="modal-backdrop" @click.self="handleClose">
    <div class="modal-card">
      ...
      <label class="field">
        <span class="label">Name der Liste *</span>
        <input
          v-model="name"
          class="input"
          type="text"
